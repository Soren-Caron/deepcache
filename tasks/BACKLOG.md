...
- [x] **M2-4b** Enemy→player damage — `core/sim/EnemyAttack.luau` + `server/services/EnemyCombatService.luau`. *Accept:* Studio — a player standing in contact loses health; cadence matches each kind's `atkCooldown`; ranged kinds strike at `atkRange`; death reaches the run lifecycle.
  **A gap with no backlog entry at all — not deferred, never written.** `EntityService` transitioned entities to `"attack"`, zeroed their velocity, faced them at the target, and returned. `Enemies.dmg` and `Enemies.atkCooldown` were read by **nothing anywhere in `src/`**, and no server code touched player Humanoid health. A player could stand in a crowd indefinitely. Reported by the user after two minutes in-game without taking a hit.
  **Why M2-4 passed over it:** that task's criterion was "one of each enemy spawns and does what its row says", verified by `rosterProbe` measuring *distance closed*. Its recorded output reads `Skitter SwarmNearest state=attack dist 70.0 -> 4.0`. That is a true statement about arriving and silent about what happens next — the same shape of hole as M2-8's unwired budget.
  **Design rules the tests encode.** *Arriving is not attacking:* an entity must serve a full cooldown before its first strike, or a swarm that closes simultaneously deletes a player in one tick with no reaction window. *Cooldowns advance from `now`, not from the missed deadline:* otherwise an entity held out of range for ten seconds banks ten cooldowns and discharges them in ten consecutive ticks on contact. *Cooldown entries are reaped:* a table keyed by entity id that only grows is a leak a play session never surfaces but a 12-minute run at 40 entities does.
  **Set `entity.targetId`, which had existed on `Types.Entity` since M1 and was never assigned.** `nearestTarget`/`heaviestTarget` returned a position only, so a strike would have landed on whoever was nearest at damage time rather than on the player the entity actually chose and is facing.
  *Verified in Studio, live run:* Skitter solo — windup held 0.8 s, then 4 strikes × 8 = 32 damage. Lancer solo — windup correctly held the longer 1.2 s, 3 strikes × 14 = 42 damage, **struck from 54 studs**, so ranged `atkRange` is respected rather than everything being melee. 3 Skitters — 13 strikes, 104 damage, player killed, `downs=1`. `pendingCooldowns` tracked entity count exactly and returned to 0 after removal. New `enemyCombat` phase costs **0.016 ms mean**; tick p95 **0.30 ms against a 12 ms budget, 0 overruns**.
  **Closes the run-ending path, which had never executed.** Killing the player moved the run from `died=0 active=1 complete=false` to `died=1 active=0 complete=true endReason="squadResolved"`. `RunLifecycleService` has bound `humanoid.Died` since M2, but nothing could kill a player, so loot-dropping, insurance claims and the risk half of the economy had never run in a real session.
  **Measured, and worth knowing before reading the numbers:** `damageDealt` legitimately exceeds observed HP drop over any window longer than a second — Roblox's default character health regeneration ticks 1%/s of MaxHealth between strikes. Confirmed rather than assumed: at t=2.5 s and t=3.0 s the resolver had dealt an identical 72 damage while HP *rose* from 29.0 to 30.0. The probe reports both numbers so this reads as regen rather than as an accounting bug.
  *Not built:* no damage feedback on the client — no hit indicator, no directional cue, no sound. From the player's seat, health simply drops. Belongs with the audio/effects work (M7-8).

- [x] **M2-4c** Enemy placement and facing — reported from a play session as "positioning is weird and they rapidly rotate". Three independent causes behind one screenshot.
  **Buried to the waist.** `LevelService.spawnPointsFor` returns points at `y = 0`, the floor plane, and consumers offset — `LootService` already did its own `+1.6`. But an entity's position is its *centre*: the hit-detection sphere is centred there and the client centres its part there too. Spawning at the floor sank half of every entity through it, and left the top half of the visible body outside its own hitbox. `WaveService.grounded` now lifts by `hitboxRadius`, so the sphere rests tangent to the floor. *Verified:* Skitter `y=1.60` against `hitboxRadius` 1.60, Sentry `y=2.20` against 2.20.
  **Spinning.** `Steering.facingYaw` assigned yaw straight from `atan2` with no rate limit, so an entity could reverse facing completely between two ticks — at 20 Hz that reads as a spin, not a turn. Compounding it, the only guard on a degenerate direction was `EPSILON = 1e-6`: a 0.3-stud offset is numerically fine but *directionally* violent, swinging tens of degrees per tick as the target shuffles. Added `Steering.slewYaw` (rate-limited, short-way-round across the seam) and a `minMagnitude` floor on `facingYaw`; per-kind `turnRate` in `config/Enemies` orders it by mass — a Skitter whips round at 8 rad/s, a Hauler at 1.2.
  **Standing inside the player.** The attack branch did `vel = 0; continue`, so `Steering.separate` — which exists and is tested — never ran while attacking. A crowd that a moving player walked into stayed interpenetrated forever because nothing was left to push it apart. Attackers now separate from neighbours *and* from the target at `ATTACK_SHUFFLE_FRACTION` of their speed.
  *Verified in Studio, 8 Skitters swarming a live player:* worst angular rate **1.828 rad/s against an 8.0 rad/s cap**; steady-state closest approach **4.65 studs against a 4.1-stud personal space**; closest entity pair 2.18 studs; entities grounded exactly at `hitboxRadius`. Screenshot confirms solid cubes resting on the floor rather than flat squares clipping through it.
  **A measurement I had to redo.** The first pass reported "2.0000 rad turned in one tick" against a 0.4 cap — apparently a 5x breach. It was exactly 5 x 0.4, which is the tell: the poll loop was aggregating five server ticks per sample. Re-measured as an angular *rate* against wall-clock elapsed time rather than assuming one tick per sample, it is 1.828 rad/s. The cap was never breached; the measurement was.
  **New tunables in `config/Combat.luau`,** deliberately a separate file: several services iterate `config/Enemies` as `for kind, config in Enemies`, so a non-kind key there would be read as an enemy called `PLAYER_RADIUS` and spawned.
  *Known and not fixed:* the client draws a fixed 3-stud box for every kind, because the snapshot has no room for a kind byte inside the 11-byte-per-entity budget. Kinds whose `hitboxRadius` is far from 1.5 therefore sit visually high or low. Resolves with per-kind rigs (M7-3/M7-6), not with a netcode change.
  *Found while testing, not fixed:* a run ends permanently on player death (`endReason="squadResolved"`) with no restart path, so a single-player Studio session is one death and done — the wave scheduler then correctly reports `run_inactive` forever. Fine as designed for an extraction shooter; awkward for playtesting, and worth a `runRestart` diagnostic.
